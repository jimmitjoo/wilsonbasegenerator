...
class bootstrap_custom_menu extends WP_Widget
{
    ...
    function bootstrap_custom_menu()
    {
        ...
        $widget_ops = array('classname' => 'bootstrapwidgetmenu', 'description' => __('A custom menu widget that uses the wp_bootstrap_navwalker', 'wbt'));

        $control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'bootstrap-widget-menu');

        parent::__construct(
            'bootstrap-widget-menu',
            __('Boot Strap Menu', 'wbt'),
            $widget_ops,
            $control_ops
        );
    }
    ...
    function form($instance)
    {
        ...
        $defaults = array('title' => __('Menu', 'wbt'), 'menuname' => __('Menu Name', 'wbt'), 'menutype' => 'nav-tabs');
        $instance = wp_parse_args((array)$instance, $defaults);
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'wbt'); ?>:</label>
            <input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>"
                   value="<?php echo $instance['title']; ?>" style="width:100%;"/>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('menuname'); ?>"><?php _e('Menu', 'wbt'); ?>:</label>
            <select id="<?php echo $this->get_field_id('menuname'); ?>"
                    name="<?php echo $this->get_field_name('menuname'); ?>" style="width:100%;" type="text">
                <?php

                $menus = get_terms('nav_menu', array('hide_empty' => false));

                echo "<option value=''>" . __('Default All Pages', 'wbt') . "</option>";
        ...
        <p>
            <label for="<?php echo $this->get_field_id('menutype'); ?>"><?php _e('Menu Type', 'wbt'); ?>
                :</label>
            <select id="<?php echo $this->get_field_id('menutype'); ?>"
                    name="<?php echo $this->get_field_name('menutype'); ?>" style="width:100%;" type="text">
                <?php
